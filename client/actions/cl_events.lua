...
RegisterNetEvent("Admin:Bennys", function(Result)
    ...
    TriggerServerEvent('exter-adminmenu/server/open-bennys', Result['player'])
end)

RegisterNetEvent("Admin:Kill", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/kill', Result['player'])
end)

RegisterNetEvent("Admin:Set:Environment", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/set-environment', Result['weather'], Result['hour'], Result['minute'])
end)

RegisterNetEvent("Admin:Delete:Area", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/delete-area', Result['type'], Result['radius'])
end)

RegisterNetEvent("Admin:Infinite:Ammo", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/toggle-infinite-ammo', Result['player'])
end)

RegisterNetEvent("Admin:Infinite:Stamina", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/toggle-infinite-stamina', Result['player'])
end)

RegisterNetEvent("Admin:Cloak", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/toggle-cloak', Result['player'])
end)

RegisterNetEvent("Admin:Godmode", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/toggle-godmode', Result['player'])
end)
...
RegisterNetEvent('Admin:Teleport', function(Result)
     if not IsPlayerAdmin() then return end
    TriggerEvent('exter-adminmenu/client/force-close')
    TriggerServerEvent('exter-adminmenu/server/teleport-player', Result['player'], Result['type'])
end)

RegisterNetEvent("Admin:Chat:Say", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/chat-say', Result['message'])
end)

RegisterNetEvent('Admin:Open:Clothing', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerEvent('exter-adminmenu/client/force-close')
    TriggerServerEvent('exter-adminmenu/server/open-clothing', Result['player'])
end)

RegisterNetEvent('Admin:Revive:Radius', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/revive-in-distance', Result['radius'])
end)

RegisterNetEvent('Admin:Revive', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/revive-target', Result['player'])
end)

RegisterNetEvent('Admin:Remove:Stress', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/remove-stress', Result['player'])
end)
...
RegisterNetEvent('Admin:Change:Model', function(Result)
    ...
    if Result['model'] ~= '' then
        ...
        if IsModelValid(Model) then
            TriggerServerEvent('exter-adminmenu/server/set-model', Result['player'], Model)
        end
    end
end)
...
RegisterNetEvent('Admin:Reset:Model', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/reset-skin', Result['player'])
end)

RegisterNetEvent('Admin:Armor', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/set-armor', Result['player'])
end)

RegisterNetEvent('Admin:Food:Drink', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/set-food-drink', Result['player'])
end)

RegisterNetEvent('Admin:Request:Gang', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/request-gang', Result['player'], Result['gang'] ~= '' and Result['gang'] or 'none')
end)

RegisterNetEvent('Admin:Request:Job', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/request-job', Result['player'], Result['job'] ~= '' and Result['job'] or 'unemployed')
end)

RegisterNetEvent("Admin:Drunk", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/drunk', Result['player'])
end)

RegisterNetEvent("Admin:Animal:Attack", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/animal-attack', Result['player'])
end)

RegisterNetEvent('Admin:Set:Fire', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/set-fire', Result['player'])
end)

RegisterNetEvent('Admin:Fling:Player', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/fling-player', Result['player'])
end)

RegisterNetEvent("Admin:Freeze:Player", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/freeze-player', Result['player'])
end)

RegisterNetEvent('Admin:SetMoney', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/set-money', Result['player'], Result['moneytype'], Result['amount'])
end)

RegisterNetEvent('Admin:GiveMoney', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('exter-adminmenu/server/give-money', Result['player'], Result['moneytype'], Result['amount'])
end)

RegisterNetEvent('Admin:GiveItem', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('exter-adminmenu/server/give-item', Result['player'], Result['item'], Result['amount'])
end)

RegisterNetEvent('Admin:Ban', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/ban-player', Result['player'], Result['expire'], Result['reason'])
end)

RegisterNetEvent('Admin:Unban', function(Result)
     if not IsPlayerAdmin() then return end
    TriggerServerEvent("exter-adminmenu/server/unban-player", Result['player'])
end)

RegisterNetEvent('Admin:Kick', function(Result)
     if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/kick-player', Result['player'], Result['reason'])
end)

RegisterNetEvent('Admin:Kick:All', function(Result)
    if not IsPlayerAdmin() then return end
   TriggerServerEvent('exter-adminmenu/server/kick-all-players', Result['reason'])
end)
...
RegisterNetEvent("Admin:Fart:Player", function(Result)
    if not IsPlayerAdmin() then return end
    TriggerServerEvent('exter-adminmenu/server/play-sound', Result['player'], Result['fart'])
end)

RegisterNetEvent('Admin:Toggle:PlayerBlips', function()
    if not IsPlayerAdmin() then return end
    BlipsEnabled = not BlipsEnabled
    TriggerServerEvent('exter-adminmenu/server/toggle-blips')
    SendNUIMessage({
        Action = "SetItemEnabled",
        Name = 'playerblips',
        State = BlipsEnabled
...
RegisterNetEvent('Admin:Toggle:Spectate', function(Result)
    ...
    if not SpectateEnabled then
        TriggerServerEvent('exter-adminmenu/server/start-spectate', Result['player'])
    else
        ToggleSpectate(StoredTargetPed)
        TriggerServerEvent('exter-adminmenu/server/stop-spectate')
    end
end)
...
RegisterNetEvent("exter-adminmenu/client/delete-area", function(Type, Radius)
    Type = Type:lower()
...
RegisterNetEvent("exter-adminmenu/client/freeze-player", function(Bool)
    FreezeEntityPosition(GetPlayerPed(PlayerId()), Bool)
end)

RegisterNetEvent("exter-adminmenu/client/toggle-infinite-ammo", function(Bool)
    while Bool do
        Wait(1)
...
RegisterNetEvent("exter-adminmenu/client/toggle-infinite-stamina", function(Bool)
    while Bool do
        Wait(1)
...
RegisterNetEvent("exter-adminmenu/client/toggle-cloak", function(Bool)
    if Bool then
...
RegisterNetEvent("exter-adminmenu/client/toggle-godmode", function(Bool)
    while Bool do
        Wait(1)
...
RegisterNetEvent('exter-adminmenu/client/teleport-player', function(Coords)
    local Entity = PlayerPedId()    
    SetPedCoordsKeepVehicle(Entity, Coords.x, Coords.y, Coords.z)
end)

RegisterNetEvent('exter-adminmenu/client/set-model', function(Model)
    QBCore.Functions.LoadModel(Model)
    SetPlayerModel(PlayerId(), Model)
    SetPedComponentVariation(PlayerPedId(), 0, 0, 0, 0)
end)

RegisterNetEvent('exter-adminmenu/client/armor-up', function()
    SetPedArmour(PlayerPedId(), 100.0)
end)

RegisterNetEvent("exter-adminmenu/client/play-sound", function(Sound)
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, Sound, 0.3)
end)

RegisterNetEvent('exter-adminmenu/client/DeletePlayerBlips', function()
    if not IsPlayerAdmin() then return end
    DeletePlayerBlips()
end)

RegisterNetEvent('exter-adminmenu/client/UpdatePlayerBlips', function(Data)
    if not IsPlayerAdmin() then return end
    BlipData = Data
end)
