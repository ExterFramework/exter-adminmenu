...
function SetKvp(Name, Data, SetData)
    ...
    SetResourceKvp('exter-adminmenumenu-'..Name, json.encode(Config.ResourceKVPs[KvpId]['Value']))
    Wait(100)
    RefreshMenu(Name)
end
...
function ResetMenuKvp()
    for i = 1, #Config.ResourceKVPs do
        SetResourceKvp("exter-adminmenumenu-" .. Config.ResourceKVPs[i]['Name'], "[]")
...
function RefreshMenu(Type)
    ...
    if Type == 'all' then -- Generate All
        for i=1, #Config.ResourceKVPs do
            ...
            if GetResourceKvpString("exter-adminmenumenu-"..KVP['Name']) == nil or GetResourceKvpString("exter-adminmenumenu-"..KVP['Name']) == "[]" then
                KVP['Value'] = GenerateKVP(KVP['Name'])
                SetResourceKvp("exter-adminmenumenu-"..KVP['Name'], json.encode(KVP['Value']))
            else
                KVP['Value'] = json.decode(GetResourceKvpString("exter-adminmenumenu-"..KVP['Name']))
            end
        end
...
function CreateLog(Type, Log, Data)
    ...
    QBCore.Functions.TriggerCallback('exter-adminmenu/server/create-log', function(result)
    end, Type, Log, Data)
end
        ...
        for i = 1, #Config.Reports do
            ...
            if Report['Id'] == ReportId then
                SetTimeout(500, function()
                    ...
                    TriggerServerEvent('exter-adminmenu/server/sync-chat-data', 'Reports', Config.Reports, 1500)
                end)
...
function GetServerConvar(Convar)
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback('exter-adminmenumenu/server/get-convar', function(ReturnData)
        Prom:resolve(ReturnData)
    end, Convar)
    return Citizen.Await(Prom)
end
...
function GetPlayersInArea(Coords, Radius)
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback('exter-adminmenu/server/get-active-players-in-radius', function(Players)
...
function GetBans()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("exter-adminmenu/server/get-bans", function(Bans)
        Prom:resolve(Bans)
    end)
    return Citizen.Await(Prom)
end
...
function GetPlayers()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("exter-adminmenu/server/get-players", function(Players)
        Prom:resolve(Players)
    end)
    return Citizen.Await(Prom)
end
...
function GetLogs()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("exter-adminmenu/server/get-logs", function(Logs)
        Prom:resolve(Logs)
    end)
    return Citizen.Await(Prom)
end
