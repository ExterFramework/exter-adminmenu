...
RegisterKeyMapping('exter_adminmenu', Lang:t('info.keymapping_desc'), 'keyboard', Config.Settings['DefaultOpenKeybind'])
RegisterCommand('exter_adminmenu', function(source, args, RawCommand) TriggerServerEvent('exter-adminmenu/server/try-open-menu', true) end, false)

RegisterNetEvent('exter-adminmenu/client/try-open-menu', function(KeyPress)
    if not IsPlayerAdmin() then return end
    if KeyPress then if not CanBind() then return end end
    ToggleMenu(false)
end)

RegisterNetEvent("exter-adminmenu/client/reset-menu", function()
    if not IsPlayerAdmin() then return end
    ResetMenuKvp()
...
RegisterNetEvent("exter-adminmenu/client/toggle-debug", function()
    if not IsPlayerAdmin() then return end
...
RegisterNetEvent("exter-adminmenu/client/do-perms-action", function(Action, CommandId, Group)
...
RegisterNUICallback("UnbanPlayer", function(Data, Cb)
    TriggerServerEvent('exter-adminmenu/server/unban-player', Data.PData.BanId)
...
RegisterNUICallback('GetCharData', function(Data, Cb)
    QBCore.Functions.TriggerCallback('exter-adminmenu/server/get-player-data', function(PlayerData)
        Cb(PlayerData)
    end, Data.Identifier)
end)
...
RegisterNUICallback("GetDateDifference", function(Data, Cb)
    QBCore.Functions.TriggerCallback('exter-adminmenu/server/get-date-difference', function(FBans, CAmount)
        Cb({
            Bans = FBans, 
            Amount = CAmount,
        })
...
RegisterNUICallback("DeleteReport", function(Data, Cb)
    ...
    if Success then
        TriggerServerEvent('exter-adminmenu/server/send-chat-report', ServerId, 
...
RegisterNUICallback('SendChatsMessage', function(Data, Cb)
    ...
    if ChatChannel == 'Staffchat' then
        ...
        TriggerServerEvent('exter-adminmenu/server/sync-chat-data', ChatChannel, Config.StaffChat, 0)
    else
        ...
        if Report then -- Check if report exists
            if Config.Reports[Report] ~= nil then
                ...
                TriggerServerEvent('exter-adminmenu/server/send-chat-report', Config.Reports[Report]['ServerId'], 
                '\n^0───────────────────────────\n^1[Admin] ^0→ ^5'..
                QBCore.Functions.GetPlayerData().name..'^0: '..ChatMessage..'\n^7→ '..
                Lang:t('info.report_reply', { chatcommand = Config.Commands['ReportChat'] })..
                '\n^0───────────────────────────')
                TriggerServerEvent('exter-adminmenu/server/sync-chat-data', ChatChannel, Config.Reports, 0)
            end
        end
    end
    Cb("Ok")
