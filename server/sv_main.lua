...
QBCore.Functions.CreateCallback('exter-adminmenumenu/server/get-permission', function(Source, Cb)
    local Group = QBCore.Functions.GetPermission(Source)
    Cb(Group)
end)

QBCore.Functions.CreateCallback('exter-adminmenumenu/server/get-convar', function(source, Cb, ConvarName)
    Cb(GetConvar(ConvarName, 'none'))
end)

QBCore.Functions.CreateCallback('exter-adminmenu/server/get-active-players-in-radius', function(Source, Cb, Coords, Radius)
...
QBCore.Functions.CreateCallback('exter-adminmenu/server/get-bans', function(source, Cb)
    local BanList = {}
...
QBCore.Functions.CreateCallback('exter-adminmenu/server/get-logs', function(source, Cb)
    local LogsList = {}
...
QBCore.Functions.CreateCallback('exter-adminmenu/server/get-players', function(source, Cb)
    local PlayerList = {}
...
QBCore.Functions.CreateCallback('exter-adminmenu/server/get-player-data', function(source, Cb, Identifier)
    local PlayerInfo = {}
    local TPlayer = nil
...
QBCore.Functions.CreateCallback('exter-adminmenu/server/get-date-difference', function(source, Cb, Bans, Type)
    local FilteredBans, BanAmount = GetDateDifference(Type, Bans) 
    Cb(FilteredBans, BanAmount)
end)

QBCore.Functions.CreateCallback("exter-adminmenu/server/create-log", function(source, Cb, Type, Log, Data)
...
RegisterNetEvent("exter-adminmenu/server/try-open-menu", function(KeyPress)
    local src = source
    if not AdminCheck(src) then return end
    TriggerClientEvent('exter-adminmenu/client/try-open-menu', src, KeyPress)
end)

-- User Commands

RegisterNetEvent("exter-adminmenu/server/unban-player", function(BanId)
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/ban-player", function(ServerId, Expires, Reason)
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/kick-all-players", function(Reason)
    local src = source
    if not AdminCheck(src) then return end
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
...
RegisterNetEvent("exter-adminmenu/server/kick-player", function(ServerId, Reason)
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/set-money", function(ServerId, MoneyType, MoneyAmount)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
...
RegisterNetEvent("exter-adminmenu/server/give-money", function(ServerId, MoneyType, MoneyAmount)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
...
RegisterNetEvent('exter-adminmenu/server/stop-spectate', function()
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/drunk", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    TriggerClientEvent('exter-adminmenu/client/drunk', Source)
end)

RegisterNetEvent("exter-adminmenu/server/animal-attack", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    TriggerClientEvent('exter-adminmenu/client/animal-attack', Source)
end)

RegisterNetEvent("exter-adminmenu/server/set-fire", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    TriggerClientEvent('exter-adminmenu/client/set-fire', Source)
end)

RegisterNetEvent("exter-adminmenu/server/fling-player", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    TriggerClientEvent('exter-adminmenu/client/fling-player', Source)
end)

RegisterNetEvent("exter-adminmenu/server/play-sound", function(ServerId, SoundId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    TriggerClientEvent('exter-adminmenu/client/play-sound', Source, SoundId)
end)

-- Utility Commands

RegisterNetEvent("exter-adminmenu/server/toggle-blips", function()
    local src = source
    if not AdminCheck(src) then return end
    local BlipData = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
    ...
    TriggerClientEvent('exter-adminmenu/client/UpdatePlayerBlips', src, BlipData)
end)


RegisterNetEvent("exter-adminmenu/server/teleport-all", function()
    local src = source
    if not AdminCheck(src) then return end
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if SourcePlayer ~= nil and TPlayer ~= nil then 
            if SourcePlayer.PlayerData.citizenid ~= TPlayer.PlayerData.citizenid then
                ...
                TriggerClientEvent('exter-adminmenu/client/teleport-player', v, SourceCoords)
            end
        end
    end
end)

RegisterNetEvent("exter-adminmenu/server/teleport-player", function(ServerId, Type)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local Msg = ""
    if Type == 'Goto' then
        ...
        TriggerClientEvent('exter-adminmenu/client/teleport-player', src, TCoords)
    elseif Type == 'Bring' then
        Msg = Lang:t('info.teleported_brought')
        local Coords = GetEntityCoords(GetPlayerPed(src))
        TriggerClientEvent('exter-adminmenu/client/teleport-player', Source, Coords)
    end
    TriggerClientEvent('QBCore:Notify', src, Lang:t('info.teleported', {tpmsg = Msg}), 'success')
end)

RegisterNetEvent("exter-adminmenu/server/chat-say", function(Message)
...
RegisterNetEvent("exter-adminmenu/server/set-environment", function(Weather, Hour, Minute)
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/open-bennys", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
...
RegisterNetEvent("exter-adminmenu/server/kill", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
...
RegisterNetEvent("exter-adminmenu/server/delete-area", function(Type, Radius)
    local src = source
    if not AdminCheck(src) then return end

    TriggerClientEvent('exter-adminmenu/client/delete-area', src, Type, Radius)
end)

RegisterNetEvent("exter-adminmenu/server/freeze-player", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
    if TPlayer ~= nil then
        ...
        TriggerClientEvent('exter-adminmenu/client/freeze-player', Source, FrozenPlayers[PData.citizenid])
    end
end)

RegisterNetEvent("exter-adminmenu/server/toggle-infinite-ammo", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
    if TPlayer ~= nil then
        ...
        TriggerClientEvent('exter-adminmenu/client/toggle-infinite-ammo', Source, AmmoEnabled[PData.citizenid])
    end
end)

RegisterNetEvent("exter-adminmenu/server/toggle-infinite-stamina", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
    if TPlayer ~= nil then
        ...
        TriggerClientEvent('exter-adminmenu/client/toggle-infinite-stamina', Source, StaminaEnabled[PData.citizenid])
    end
end)

RegisterNetEvent("exter-adminmenu/server/toggle-cloak", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
    if TPlayer ~= nil then
        ...
        TriggerClientEvent('exter-adminmenu/client/toggle-cloak', Source, CloakEnabled[PData.citizenid])
    end
end)

RegisterNetEvent("exter-adminmenu/server/toggle-godmode", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
    if TPlayer ~= nil then
        ...
        TriggerClientEvent('exter-adminmenu/client/toggle-godmode', Source, GodmodeEnabled[PData.citizenid])
    end
end)

RegisterNetEvent("exter-adminmenu/server/set-food-drink", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
...
RegisterNetEvent("exter-adminmenu/server/remove-stress", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
...
RegisterNetEvent("exter-adminmenu/server/set-armor", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
...
RegisterNetEvent("exter-adminmenu/server/reset-skin", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    local TPlayer = QBCore.Functions.GetPlayer(Source)
...
RegisterNetEvent("exter-adminmenu/server/set-model", function(ServerId, Model)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
    TriggerClientEvent('exter-adminmenu/client/set-model', Source, Model)
end)

RegisterNetEvent("exter-adminmenu/server/revive-all", function()
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/revive-in-distance", function(Radius)
    local src = source
    if not AdminCheck(src) then return end
...
RegisterNetEvent("exter-adminmenu/server/revive-target", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
...
RegisterNetEvent("exter-adminmenu/server/open-clothing", function(ServerId)
    local src = source
    if not AdminCheck(src) then return end
    local Source = ServerId ~= nil and ServerId or src
...
RegisterNetEvent('exter-adminmenu/server/sync-chat-data', function(Type, Data, UpdateDelay)
    ...
    TriggerClientEvent('exter-adminmenu/client/sync-chat-data', -1, Type, Type == 'Staffchat' and Config.StaffChat or Config.Reports, UpdateDelay)
end)

RegisterNetEvent("exter-adminmenu/server/send-chat-report", function(ServerId, Message)
